...
# ╔═╡ 11368433-f5c7-4251-aed7-81d0ae15c70f
using PlutoUI, Images,ImageIO, ImageFiltering, TestImages,Plots,LaTeXStrings, Latexify, GaussianProcesses

# ╔═╡ 7d87dff0-3a89-11ec-1b3b-ab4de607888f
md"""
### Proceso Estocástico Gaussiano

Un proceso estocástico Gaussiano $x(n)$ tiene la propiedad de que para cualquier muestra finita de $k$ puntos $\{n_{1}, n_{2}, ..., n_{k}\}$, la distribución conjunta de $x(n_{r})$ con $r = 1, 2, ..., k$ es normal k-variable.

Cabe recordar que un vector aleatorio $X = (x_{1}, x_{2}, ..., x_{k})$ distribuye normal multivariable si cualquier combinación lineal de variables aleatorias $x_{1}, x_{2}, ..., x_{k}$ distribuyen normalmente. En otras palabras:

$a_{1} x_{1} + a_{2} x_{2} + ... + a_{n} X_{n} \sim N$

que tiene un vector de media

$\mu = (E(x_{1}), E(x_{2}), ..., E(x_{n}))$

y una matriz de covarianza $\Sigma$ que satisface:

$\Sigma_{i,j} = Cov(x_{i},x_{j})$

donde su función de densidad de distribución está dada por:

$p(X; \mu, \Sigma) = \frac{1}{(2 \pi)^{\frac{n}{2}} |\Sigma|^{\frac{1}{2}}} e^{-\frac{1}{2}(X - \mu)^T \Sigma^{-1}(X-\mu)}$

Finalmente, como X está completamente definida por $\mu$ y $\Sigma$, se puede referir a ella de la siguiente forma:

$X \sim N(\mu, \Sigma)$



Un uso común de este proceso es en Machine Learning: la regresión del proceso gaussiano o GPR en inglés (Gaussian Process Regression) se usa con el objetivo de predecir el valor de cierto $x(n_{i})$ dada una muestra de datos $x(n_{1}, x_{2}, ..., x_{k})$.

Para esto se elige una función de covarianza (que, dependiendo del uso, puede ser lineal, exponencial, constante, etc.) $k(n, n')$, para la cual se calculan las covarianzas de todos los $n$ que se tienen y se resumen los valores en las siguientes matrices:

```math
  K = \left[ \matrix{
k(n_{1}, n_{1}) & k(n_{1}, n_{2}) & ... & k(n_{1}, n_{k}) \\
k(n_{2}, n_{1}) & k(n_{2}, n_{2}) & ... & k(n_{2}, n_{k}) \\
\vdots & \vdots & \ddots & \vdots \\
k(n_{k}, n_{1}) & k(n_{k}, n_{2}) & ... & k(n_{k}, n_{k})} \right]
```

```math
  K_{*} =  \left[ \matrix{k(n_{i}, n_{1}) & k(n_{i}, n_{2}) & ... & k(n_{i}, n_{k}) }\right]
```
...
```
y, dado que la suposición clave del proceso gaussiano es que los datos se pueden representar como muestras de una distribución gaussiana multivariable, se tiene que:

```math
  \left[ \matrix{x \\ x(n_{i}) }\right] \sim N \left(0, \left[ \matrix{K &  K_{*}^{T} \\   K_{*} &  K_{**} }\right] \right)
```

o más bien 

```math
  x(n_{i}) | x \sim N\left(K_{*}K^{-1} x, K_{**} - K_{*} K^{-1} K_{*}^{T}\right)
```

donde la mejor estimación para $x(n_{i})$ es la media de esta distribución:

```math
  \overline{x(n_{i})} =   K_{*}K^{-1} x
```

y la incertidumbre de la estimación está capturada en la varianza:

```math
  var(x(n_{i})) =  K_{**} - K_{*} K^{-1} K_{*}^{T}
```

Además de la predicción puntual, se puede calcular in intervalo de confianza, que se ve representado en el siguiente gráfico:
...
#definición: 
# Bhat, U. (1972). Elements of applied stochastic processes. New York: Wiley.

# gráfico:
#https://docs.juliahub.com/GaussianProcesses/izlaf/0.12.0/plotting_gps/

# distribucion normal multivariable
# https://brilliant.org/wiki/multivariate-normal-distribution/

# ╔═╡ c9fe721f-3a22-4815-880c-649a7ba7472e
begin
	...
	gp = GP(x,y,m,se)
	plot(gp;  xlabel="n", ylabel="x", title="Proceso Gaussiano", legend=false, fmt=:png)
end

# ╔═╡ 34851fba-ddb4-42a7-8386-fa817a9c26a1
md"""
Donde cada punto es una muestra, la línea negra es la predicción de cada n y la banda azul es el intervalo de confianza.
"""
