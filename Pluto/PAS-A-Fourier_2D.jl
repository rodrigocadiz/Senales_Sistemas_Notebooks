...
using PlutoUI, ImageIO, ImageFiltering, TestImages, FFTW, Images, Plots,LaTeXStrings,ImageView,Measures

# ╔═╡ 52ca889d-5600-49a7-a472-b6b36f9183bc
md"""
!!! header ""
...
# ╔═╡ 8428ba93-b35c-4d59-b14a-e3967feee767
md"""
...
md"""
# Transformada de Fourier en $\mathbb{R}^2$
"""

# ╔═╡ 51c9ac01-a7a3-401e-86d2-c9271e875e23
md"""
...
# ╔═╡ 80a8b2ed-1942-4dca-ae15-e02be06eb769
md"""
## Notación

La notación $\mathcal{F} \{f\}\left(\xi_1, \xi_2\right)$ enfatiza el hecho que la Transformada $\mathcal{F}$ se aplica a la función $f$ y se evalúa en las variables de frecuencia $\xi_1,\xi_2$. En el dominio de las imágenes es muy común utilizar las variables $x,y$ para denotar los ejes espaciales y $u,v$ para denotar frecuencias espaciales. 

Consierando lo anterior, la Transformada de Fourier en $\mathbb{R}^2$ se podría anotar de manera simplificada como
...
donde se utilizan letras mayúsculas para referirse a la Transformada de Fourier de la señal original. 

Utilizando esta notación simplificada, la definición sería 

!!! note "Definición"
	$$F(u,v) = \int_{-\infty}^{\infty} \int_{-\infty}^{\infty} e^{-2 \pi i\left(ux+vy \right)} f\left(x,y\right) dxdy$$



"""

# ╔═╡ ba64dd3b-b2ec-44db-a94b-bb57dd94d923
md"""
Como la Transformada de Fourier resulta en una función bi-dimensional compleja, al momento de graficarla es necesario especificar si se graficara su parte real o imaginaria. En general, **es más util graficar la magnitud y fase** de la Transformada de Fourier, dado que esta representación tiene una interpretación física, tal como se observa en las siguientes imágenes. 
"""

# ╔═╡ 49492041-47e4-4557-8512-de3aad207be6
begin 
	gr(size=(900,350),margin=5mm,xguidefontsize=18,yguidefontsize=18,titlefontsize=18)
	img1 = testimage("cameraman");
	imgF1 = fftshift(fft(Float64.(Gray.(img1))))
	hm1 = heatmap(log.(abs.(imgF1).+1),legend=:false,color=:grays,axis=nothing,showaxis=:false,xlabel=L"u",ylabel=L"v",title=L"\log \,\, |F(u,v)|",aspect_ratio=1.0)
	hm12 = heatmap(angle.(imgF1),legend=:false,color=:grays,axis=nothing,showaxis=:false,xlabel=L"u",ylabel=L"v",title=L"\angle F(u,v)",aspect_ratio=1.0)
		plot(plot(img1,xlabel=L"x",ylabel=L"y",title=L"f(x,y)"),hm1,hm12,axis=nothing,layout=(1,3))
end

# ╔═╡ 77f7eee3-0940-4dd1-aa29-a5159b990528
md"""
A la izquierda se observa la imagen en el dominio del tiempo, el logaritmo de la magnitud de la Transformada de Fourier en la imagen del centro y la fase en la imagen de la derecha. El logaritmo de la magnitud es útil para ver detalles de amplitudes en distintas posiciones de frecuencia. En una escala lineal, muchas veces sucede que el único punto brillante en una imagen es el origen y el resto se invisibiliza. 

!!! warning "Importante"
	Arriba, en las imágenes del centro y derecha, en el centro de la imagen está el origen que contiene la frecuencia cero. No obstante, podría escogerse alguna de las esquinas de la imagen como el origen, tal como se observa en las siguientes imagenes de abajo:
"""

# ╔═╡ 57726a32-8cbb-4218-9b71-da3b1ac65f22
begin 
	gr(size=(900,350),margin=5mm,xguidefontsize=18,yguidefontsize=18,titlefontsize=18)
	imgF3 = fft(Float64.(Gray.(img1)))
	hm3 = heatmap(log.(abs.(imgF3).+1),legend=:false,color=:grays,axis=nothing,showaxis=:false,xlabel=L"u",ylabel=L"v",title=L"\log \,\, |F(u,v)|",aspect_ratio=1.0)
	hm32 = heatmap(angle.(imgF3),legend=:false,color=:grays,axis=nothing,showaxis=:false,xlabel=L"u",ylabel=L"v",title=L"\angle F(u,v)",aspect_ratio=1.0)
		plot(plot(img1,xlabel=L"x",ylabel=L"y",title=L"f(x,y)"),hm3,hm32,axis=nothing,layout=(1,3))
end

# ╔═╡ dcb01355-6b62-4ed0-9afc-78955b11daa5
md"""
...
# ╔═╡ a82e179f-0edd-419d-9f3b-153c65d37219
md"""
En notación simplificada, la inversa sería

!!! note "Definición"
	$$f(x,y) = \int_{-\infty}^{\infty} \int_{-\infty}^{\infty} e^{2 \pi i\left(ux+vy \right)} F\left(u,v\right) dudv$$
...
# ╔═╡ df64058a-4a10-49a0-aaf4-c44e117da34e
md"""
## Dualidad
...
Utilizando la definición de la Transformada de Fourier tenemos:

$$\mathcal{F}\{f\}(-\xi_1,-\xi_2)=\int_{\mathbb{R}^{2}} e^{-2 \pi i (-x_1 \xi_1 -x_2\xi_2)} f(x_1,x_2) dx_1 dx_2$$
$$= \int_{\mathbb{R}^{2}} e^{2 \pi i (x_1 \xi_1+x_2\xi_2)} f(x_1,x_2) dx_1 dx_2 ={\mathcal{F}}^{-1} \{f\}(\xi_1,
\xi_2)$$

Observamos entonces que 

!!! note ""
	$$\mathcal{F}\{f\}(-\xi_1,-\xi_2)=\mathcal{F}^{-1}\{f\}(\xi_1,\xi_2)$$

lo que se conoce como el **principio de la dualidad** de la Transformada de Fourier.

Esta relación también puede escribirse como

!!! note ""
	$$\mathcal{F}\{\mathcal{F}\{f\}\}(x_1,x_2)=f(-x_1,-x_2)$$

o en una notación más simple

!!! note ""
	$$F(x_1,x_2) \xrightarrow{\mathcal{F}} f(-x_1,-x_2)$$

"""

# ╔═╡ 550b546c-3782-49c2-9781-f4f5d9ed21f1
md"""
## Concentración versus dispersión

Una señal con variaciones suaves en el dominio del espacio resulta en un contenido espectral concentrado en el dominio de la frecuencia. De forma contraria, variaciones espaciales abruptas dispersan la energía en el espectro, de manera que éste decae suavemente hacia las frecuencias altas.

"""

# ╔═╡ deefdda9-acc6-41d1-8158-427fcd484c20
begin 
	gr(size=(900,350),margin=2mm,xguidefontsize=18,yguidefontsize=18,titlefontsize=18)
	anim = @animate for i in 1:0.1:20
	
		img2 = testimage("monarch_color_256");
		imgg2 = imfilter(img2, Kernel.gaussian(i));
		imgF2 = fftshift(fft(Float64.(Gray.(imgg2))))
		hm = heatmap(log.(abs.(imgF2)).+1,legend=:false,color=:grays,axis=nothing,showaxis=false,xlabel=L"u",ylabel=L"v",title=L"\log |F(u,v)|",aspect_ratio=1.0)
	plot(plot(imgg2,xlabel=L"x",ylabel=L"y",title=L"f(x,y)"),hm,axis=nothing,layout=(1,2))
	end
	gif(anim,fps=2)
end
...
# ╔═╡ d40fa239-edf4-4ef8-bcbb-beff8dd0e4fa
md"""
...
# ╔═╡ 5f468f79-81f2-4a72-b6ca-1e35c0881a14
md"""
### Ejemplo: $\sqcap(x,y)$
...
# ╔═╡ 97ea79da-f726-43aa-a49f-89a422954e9b
md"""
### Ejemplo: $\mathrm{Gauss}(x,y)$

Veamos el caso de la Transformada de Fourier de un $\mathrm{Gauss}$ en $\mathbb{R}^2$:

> $\mathcal{F}\{\mathrm{Gauss}(x,y)\}(u,v) = \mathcal{F}\{\mathrm{Gauss}(x)\mathrm{Gauss}(y)\}(u,v)$ 
> $= \mathrm{Gauss}(u)\mathrm{Gauss}(v) = \mathrm{Gauss}(u,v)$

lo que nos permite calcular nuestro segundo par de Fourier en dos dimensiones:

!!! note ""
	$\mathrm{Gauss}(x,y) \rightarrow \mathrm{Gauss}(u,v)$



"""

# ╔═╡ 72e9cb02-0efb-4464-bcdc-0353d509e295
md"""
...
La integral interior es la Transformada de Fourier 1D de $f\left(x_{1}, x_{2}\right)$ con respecto a $x_{1}$. Para encontrar el resultado completo $\mathcal{F}\{ f\}\left(\xi_{1}, \xi_{2}\right)$ falta tomar otra Transformada de Fourier 1D, esta vez respecto a $x_{2}$. Esto se podría hacer en el orden inverso también.
...
# ╔═╡ 6dc9b6c4-2660-4f22-ab2c-ac008b713751
md"""
## Transformada de Fourier en $L^1, L^2$ y $S$

!!! warning "Importante" 
	Por supuesto, la integral doble de la Transformada de Fourier en $\mathbb{R}^2$ no convergerá para todas las funciones y su validez depende si las funciones pertenecen a los espacios $L^1(\mathbb{R}^2), L^2(\mathbb{R}^2)$ o si son distribuciones temperadas operando en el espacio $\mathcal{S}(\mathbb{R}^2;\mathbb{C})$. Las consideraciones y precauciones a tomar son totalmente análogas al caso uni-dimensional. 

"""

# ╔═╡ 625ca673-3e3b-4a0e-97d2-bc10ca93937a
md"""
***
[[Volver a la tabla de contenidos](Contenidos.jl.html)]
"""

# ╔═╡ 8dba51a8-d54f-463b-8b4a-95c88bb0d95d
md"""
!!! footer ""
